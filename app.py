...
GLOSSARY_V2 = {
    ...
    "💫 Scores": [
        ...
        ("Confidence vs Board", "Confidence Score is the 0–100 on Confidence. Board score is the ticket stack on Run It. Confidence never greens a ticket by itself."),
        ("Data Score", "0–100 data quality. MLB: EV, HH%, barrel, xSLG, HR L7, heating. NFL: targets, receptions, yards, TDs, role, volume, trend. Data Score ≥ 70 auto-tags Heating."),
        ("Odds Score", "0–100 odds agreement. Multi-book alignment, tight prices, movement toward the pick, boosts, market confidence. Odds Score ≥ 70 auto-tags Rhythm."),
        ("Context Score", "0–100 context strength. MLB: park, pitcher, wind, weather, home/road. NFL: defense, home/road, primetime, dome, coverage. Context Score ≥ 70 auto-tags Petty Upside."),
        ("Petty Upside / Edge", "Tag when Context Score ≥ 70. Also the loud-data footer. Context is cooking, not a ticket by itself."),
        ("Heating tag", "Fires when Data Score ≥ 70 or Confidence jumped 10+ vs the last look. Pink tag. Not the same as HR L7 heating in the data line."),
        ("Rhythm tag", "Fires when Odds Score ≥ 70. Books clustered / tight / moving with us. Teal tag."),
        ("Petty Upside tag", "Fires when Context Score ≥ 70. Park, wind, pitcher, or matchup is loud. Purple tag."),
        ("HOT tier", "Confidence 70–100. Active Board. Pink→purple border, shimmer, glow pulse, animated bar. This pick is giving everything. Max 12 cards."),
        ("MID tier", "Confidence 50–69. Whispers. Teal border, soft glow, static bar. Good spot, not a must. Rhythm tag only."),
        ("COLD tier", "Confidence 0–49. Homework. Gray border, no glow, muted bar. Board says no."),
        ("Active / Whispers / Homework", "Active Board = HOT 70–100, max 12. Whispers = MID 50–69. Homework = COLD 0–49. Same buckets on the three count boxes."),
        ("Hot / Mid / Cold data", "Same as HOT / MID / COLD Confidence tiers now: 70–100 / 50–69 / 0–49. Old flag-count pile is retired. Active still max 12."),
...
def render_run_it_recap():
    ...
    for r in rows:
        ...
        if sport == "NFL":
            ...
            if src.startswith("need_one") or any(x in mkt for x in ("rush", "receiv", "reception", "yard")):
                continue
        if src in ("take_it", "shop_take", "watch", "shop_lean", "bet_this", "take"):
            keep.append(r)
        elif res in ("HIT", "MISS"):
            keep.append(r)
    # Frozen ledger takes survive a Cloud wipe of the live board.
    have = {_fold_player(r.get("player")) for r in keep}
    try:
        for row in (load_take_ledger() or {}).get(today, []) + (load_take_ledger() or {}).get(extra_day, []):
            if str(row.get("sport") or active_sport()) not in (active_sport(), "", None):
                if row.get("sport") and row.get("sport") != active_sport():
                    continue
            k = _fold_player(row.get("player"))
            if not k or k in have:
                continue
            keep.append({
                "player": row.get("player"),
                "source": row.get("source") or "take_it",
                "result": "PENDING",
        ...
        pass
    if not keep:
        st.caption("Nothing logged today yet. Fetch pregame so TAKE / WATCH / LEAN land here.")
        return
    hits_n = sum(1 for r in keep if str(r.get("result") or "").upper() == "HIT")
    take_n = sum(1 for r in keep if str(r.get("source") or "") in ("take_it", "shop_take", "bet_this", "take"))
    if hits_n:
        queen = f"Queen says: {hits_n} actually went."
    elif take_n:
        queen = f"Queen says: {take_n} tickets are logged. Games still live — Hits is empty until they grade."
    else:
        queen = "Queen says: research only so far. Nothing is a ticket until it was TAKE."
    st.markdown(f'<div class="wg-queen" style="text-align:left">{queen}</div>', unsafe_allow_html=True)
    slice_r = st.radio(
        "recap_slice",
        ["Tickets only", "Research only", "Hits only", "Misses only", "All unique names"],
        horizontal=True,
        key="recap_slice_v2",
        label_visibility="collapsed",
    )
    ...
    def _result(r):
        ...
        return "STILL UP", "⏳ STILL UP", "r-watch", 2
...
def main():
    ...
    if page == "Lines:Lock":
        ...
        st.caption(
            "One row per unique name. Tickets = TAKE. Research = WATCH / LEAN. "
            "Did it go? is HIT / MISS / STILL UP — never another LEAN. "
            "Hits only is empty until box scores grade."
        )
        render_run_it_recap()
