...
LOCKED method weights (from 14-day graded data):
  PREMIUM core → FD 600, Multi-book Shorten, Match/MGM 25·50·75,
                 MGM Exact, Stayed in the group, Last one left
  SUPPORT only (tag, no TAKE IT credit) → Books tight, Exact Match,
                 broad FD Pattern, DK FD-style
  NOISE → DK 10, MGM/Match 00, vague multi-book, Multi-book Lengthen
  SPECIAL → MGM 75 + (Stayed OR Last one left) → TAKE IT even if edge soft

Also: pregame lock · Results undo · Log HR · glossary
"""
...
<style>
    ...
    .watch-card { border-color: #c084fc !important; }
    ...
    .tag-support { background: #1e1b4b; color: #c4b5fd; border-color: #8b5cf6; }
    .tag-noise { background: #1f2937; color: #9ca3af; border-color: #4b5563; }
...
EDGE_MIN = 60          # normal TAKE IT edge
EDGE_SOFT = 20         # soft floor for MGM 75 + sticky bypass
METHODS_MIN = 2
OUTLIER_GAP = 150
REFRESH_MINUTES = 30
NAME_METHODS_MIN = 3
...
# ── LOCKED WEIGHTS ───────────────────────────────────────────
# Premium = counts toward 2+ core for TAKE IT
PREMIUM_CORE = {
    "FD 600",
    "Multi-book Shorten",
...
# Support = show as tags, do NOT count toward TAKE IT core
SUPPORT_ONLY = {
    "Books tight",
    "Exact Match",
    "FD Pattern",
...
def is_premium_core(m):
    """Only these count toward METHODS_MIN / TAKE IT."""
    m = str(m)
    if m in NOISE_METHODS or m in SUPPORT_ONLY:
        return False
    if m in PREMIUM_CORE:
        return True
    # "Stayed in group 3x" etc.
    if m.startswith("Stayed in group"):
        return True
    # "MGM 25" style already in set; catch "MGM 75" variants
    if m.startswith("MGM ") and any(x in m for x in ("25", "50", "75")) and "00" not in m:
        return True
    return False

def is_core_method(m):
    """Alias used everywhere — premium only."""
    return is_premium_core(m)

def count_core_methods(meths):
    return len({m for m in meths if is_premium_core(m)})

def has_personal_strong(meths):
    return any(
        m in PERSONAL_STRONG or m.startswith("Match ") or m.startswith("B365")
        or m.startswith("Stayed in group")
        for m in meths
    )

def has_dk_or_mgm(meths):
    """Used to gate FanDuel pattern display (still show FD tags)."""
    for m in meths:
        if m == "DK 10":
            return True
        if is_premium_core(m) and ("MGM" in m or "Match" in m or "Stayed" in m or "Last one" in m):
            return True
    return False

def has_mgm_75_sticky(meths):
    """SPECIAL: MGM 75 / Match 75 + (Stayed OR Last one left)."""
    has_75 = any(m in ("Match 75", "MGM 75") for m in meths)
    has_sticky = any(
        m == "Last one left" or m == "Stayed in the group" or str(m).startswith("Stayed in group")
        for m in meths
    )
    return has_75 and has_sticky

def method_tier(m):
    m = str(m)
    if is_premium_core(m):
        return "premium"
    if m in SUPPORT_ONLY:
        return "support"
    if m in NOISE_METHODS or m == "DK 10" or "00" in m:
        return "noise"
    return "support"
...
def method_tag_class(m):
    ...
    if m.startswith("MGM") or m == "Last one left" or "Stayed in" in m:
        return "tag-mgm"
    ...
    if "Multi-book Shorten" in m: return "tag-strong"
...
def girl_magic_score(core_count, edge, methods):
    ...
    if "FD 600" in methods: bonus += 4
    if any(m in ("Match 75", "MGM 75") for m in methods): bonus += 2
...
def get_confidence(score, is_bet):
    if not is_bet: return "Watch", 1, "low"
...
def prices_are_tight(prices, max_spread=40):
    """Books tight = clustered prices (support tag only)."""
    if len(prices) < 2:
        return False
    return (max(prices) - min(prices)) <= max_spread and len(set(prices)) > 1
...
def run_flags(df, previous_df=None, record_history=True, selected_events=None):
    ...
    for player, g in df.groupby("player"):
        ...
        if fd is not None and others and fd > max(others):
            results.append({
                "type": "trend", "trend_kind": "fade", "label": player,
                "reason": f"🔴 FD highest of all books · FD {format_odds(fd)}",
    ...
    # DK 10 — still FLAG for WATCH, but NOISE for TAKE IT scoring
    for _, row in df.iterrows():
        if row["book"] == "draftkings" and last_two(row["price"]) == 10:
            results.append({"type": "dk", "label": row["player"],
                "reason": f"DraftKings ends in 10 → {format_odds(row['price'])} (WATCH only · benched for TAKE IT)",
    ...
    # MGM groups (00 still flagged but NOT premium core)
    ...
    for grp in current_mgm:
        ...
        # MGM 00 is noise for scoring; 25/50/75 premium
        meth = [f"MGM {d:02d}"]
        extra = []
        for n in names:
    ...
    # Digits 25/50/75 only (00 not Match-tagged as premium)
    for (event, team), g in mgm.groupby(["event", "team"], dropna=False):
        ends = defaultdict(list)
        for _, r in g.iterrows():
            d = last_two(r["price"])
    ...
    # FanDuel — FD 600 premium; broad pattern support only
    for _, row in df.iterrows():
        ...
        if price == 600:
            results.append({"type": "fd", "label": player,
                "reason": f"FanDuel exact +600 → {format_odds(row['price'])} 🟢 PREMIUM",
                "event": row["event"], "css": "fd", "methods": ["FD 600"]})
            methods_map[player].append("FD 600")
        elif price >= FD_MIN and last in (10, 20, 30, 60, 70, 90):
            # only show if already has some MGM/DK context optional
            results.append({"type": "fd", "label": player,
                "reason": f"FanDuel ≥ +{FD_MIN} ends in {last:02d} (support)",
        ...
        if len(prices) >= 3:
            for i, (p, b) in enumerate(zip(prices, books)):
                ...
                if p - med_rest >= OUTLIER_GAP:
                    signal_bucket[player].append(f"One higher: {b} at {format_odds(p)}")
    ...
    if len(phist) >= 3:
        for key in phist[-1].keys():
            ...
            if len(vals) >= 3 and len(set(vals)) == 1:
                signal_bucket[player].append(f"Stuck on {book} at {format_odds(vals[0])}")
    ...
    # ── BOARD: TAKE IT vs WATCH with locked weights ──
    ev_board = []
    ...
    for (player, _), g in df.groupby(["player", "point"], dropna=False):
        ...
        sticky75 = has_mgm_75_sticky(meths)

        # TAKE IT rules (locked):
        # A) 2+ premium core AND edge >= 60
        # B) MGM 75 + (Stayed OR Last one left) AND edge >= soft floor
        normal_take = core_count >= METHODS_MIN and edge >= EDGE_MIN
        soft_take = sticky75 and edge >= EDGE_SOFT
        is_bet = normal_take or soft_take

        # Show on board if any premium OR soft sticky OR enough tags to watch
        show = is_bet or core_count >= 1 or sticky75
        if not show:
            continue

        display_meths = meths  # show all tags; scoring only uses premium
        score = girl_magic_score(core_count, edge, [m for m in meths if is_premium_core(m)])
        conf, bars, level = get_confidence(score, is_bet)
        if is_bet and soft_take and not normal_take:
            why = f"Score {score}/100 · MGM 75 + sticky · soft edge {int(edge)} (floor {EDGE_SOFT}). TAKE IT."
        elif is_bet:
            why = f"Score {score}/100 · {core_count} premium · Edge {int(edge)}. TAKE IT."
        else:
            why = f"WATCH · {core_count} premium · Edge {int(edge)} (need {EDGE_MIN}+ or 75+sticky)."
        ...
        ev_board.append({
            ...
            "soft_take": soft_take and is_bet,
        })
    ...
    for player, old in prev_ev.items():
        ...
        if player not in all_players_now:
            ...
            reasons.append(f"Lost premium methods ({now_core} left)")
        else:
        ...
        fallen.append({
            ...
            "event": "", "css": "hist",
            "methods": ["Fallen Off"] + (["Was TAKE IT"] if old.get("is_bet") else []),
    ...
    # Name magic — still requires premium + personal strong
    pev = defaultdict(set)
    for _, r in df.iterrows():
...
def main():
    ...
    st.markdown(f"""
    <div class="how-to">
        🔒 Method weights <b>LOCKED</b> from graded data · premium core only for TAKE IT<br>
        🟢 <b>FD 600 · Multi-book Shorten · Match/MGM 25·50·75 · MGM Exact · Stayed · Last one left</b><br>
        👀 Support (no TAKE IT credit): Books tight · Exact Match · FD Pattern · DK 10 benched<br>
        ⚡ <b>MGM 75 + sticky</b> soft-passes edge (floor {EDGE_SOFT}) · lock players: <b>{lock_n}</b>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        if st.button("📋 Refresh RotoWire Lineups"):
            with st.spinner("Pulling RotoWire…"):
    ...
    counts = {
        ...
        "watch": len([e for e in ev_board if not e["is_bet"]]),
    ...
    <div class="petty-row">
        ...
        <div class="petty-box"><div class="petty-num">{counts['watch']}</div><div class="petty-label">👀 WATCH</div></div>
    ...
    with tabs[0]:
        st.markdown('<div class="queen-banner">👑 The Board · locked weights</div>', unsafe_allow_html=True)
        st.caption("🟢 TAKE IT = 2+ premium + edge≥60 · OR MGM 75+sticky with soft edge · 👀 rest = WATCH")
        if not ev_board:
            ...
            for idx, item in enumerate(ev_board):
                with cols[idx % 2]:
                    ...
                    if item["is_bet"]:
                        cls, label = "bet", "🟢 TAKE IT"
                        if item.get("soft_take"):
                            label = "🟢 TAKE IT · 75 sticky"
                    else:
                        cls, label = "watch-card", "👀 WATCH"
                    ...
                    <div class="card {cls} grid-card">
                        ...
                        <b>Edge:</b> {int(item["edge"])} · <b>Premium:</b> {item.get("method_count", 0)}<br>
    ...
    show(tabs[1], "dk", "🎯 DK 10s (benched for TAKE IT)", "Still listed for WATCH · 0% on graded TAKE IT.")
    show(tabs[2], "mgm", "🎰 BetMGM Magic", "25/50/75 premium · 00 noise · Stayed / Last one left premium.")
    show(tabs[3], "match", "🤝 Exact Match (support)", "Tag only — does not count toward TAKE IT core.")
    show(tabs[4], "mgm_exact", "⭐ MGM Exact", "Premium core.")
    show(tabs[5], "digit", "🔢 Digits", "Match 25/50/75 premium · same team pairs/groups.")
    show(tabs[6], "fd", "💙 FanDuel", "FD 600 = premium · broad pattern = support only.")
    show(tabs[7], "b365", "💚 Bet365", "Support / personal strong when available.")

    with tabs[8]:
        st.markdown('<div class="queen-banner">📈 Signals</div>', unsafe_allow_html=True)
        st.caption("Multi-book Shorten = premium · Lengthen = noise · Books tight = support")
    ...
    with tabs[13]:
        ...
        st.caption("Merge-only. Survives MGM drop after live.")
        lock = st.session_state.get("pregame_lock") or load_pregame()
        if not lock:
    ...
    show(tabs[14], "same_init", "💅 Same Initials", "Different teams · needs premium.")
    ...
    with tabs[19]:
        ...
        st.caption("Wrong HIT? → Recent graded → ↩️ Undo")
        ...
        if method_stats:
            ...
            for m, s in sorted(method_stats.items(), key=lambda x: -(x[1]["hit"] / max(1, x[1]["hit"] + x[1]["miss"]))):
                ...
                tier = method_tier(m)
                lines.append(f"**{m}** ({tier}): {s['hit']}/{t} ({pct:.0f}%)")
        ...
        if not slice_p:
            ...
            for r in slice_p:
                ...
                st.markdown(
                    ...
                    f"{book_label(r.get('best_book'))}{end_s} · score {r.get('score')} · {r.get('time', '')}{extra}"
                )
                st.caption(", ".join((r.get("methods") or [])[:6]))
    ...
    with tabs[20]:
        st.markdown('<div class="queen-banner">📖 The Code · locked weights</div>', unsafe_allow_html=True)

        st.markdown("### Board")
        with st.expander("🟢 TAKE IT vs 👀 WATCH", expanded=True):
            st.markdown(f"""
**TAKE IT** when either:
1. **2+ premium** methods **and** edge ≥ **{EDGE_MIN}**, or  
2. **MGM 75 / Match 75** **and** (**Stayed in the group** or **Last one left**) **and** edge ≥ **{EDGE_SOFT}** (soft pass)

**WATCH** = has signal but doesn’t meet TAKE IT.

Score 0–100 ranks the list. Only **premium** methods raise the core count.
            """)
        with st.expander("📊 Edge"):
            st.markdown(f"**Best − median** (outlier ≥{OUTLIER_GAP} ignored). Normal floor **{EDGE_MIN}**. Soft floor **{EDGE_SOFT}** only for 75+sticky.")

        st.markdown("### 🟢 Premium (count toward TAKE IT)")
        with st.expander("Premium list", expanded=True):
            st.markdown("""
- **FD 600**
- **Multi-book Shorten**
- **Match 25 / 50 / 75** and **MGM 25 / 50 / 75**
- **MGM Exact**
- **Stayed in the group** / Stayed in group Nx
- **Last one left**
            """)

        st.markdown("### 👀 Support (tag only)")
        with st.expander("Support list"):
            st.markdown("""
- **Books tight** — prices clustered; common on WATCH, weak on TAKE IT  
- **Exact Match**  
- **FD Pattern** (broad endings) — **600** is the premium one  
- **Same on 3+ books**  
- Bet365 tags when available  
            """)

        st.markdown("### 🔴 Noise / benched")
        with st.expander("Noise list"):
            st.markdown("""
- **DK 10** — 0% on graded TAKE IT (still shown on DK tab for WATCH)  
- **MGM 00 / Match 00**  
- **Multi-book Lengthen**  
- Late / lineup / FADE tags  
            """)

        st.markdown("### Lock · Results · Lineups")
        with st.expander("🔒 Pregame lock"):
            st.markdown("Every Fetch **merges** prices. When MGM vanishes after live, last numbers **stay**. Use for Log a HR.")
        with st.expander("📊 Results"):
            st.markdown("PENDING never erased when games go live. **Oldest first**, page through all. **↩️ Undo** on graded (TAKE IT → PENDING · manual HR deleted).")
        with st.expander("📋 RotoWire"):
            st.markdown("Non-starters filtered off The Board when lineups loaded.")

        st.markdown("### Name Magic")
        with st.expander("Initials & names"):
            st.markdown(f"Both need **{NAME_METHODS_MIN}+ premium** + personal strong · **different teams** · max {NAME_MAX_PAIRS}.")
