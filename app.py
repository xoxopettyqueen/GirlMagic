...
📈 Movement ordered:
  Likes (FD under MGM + good downs) → Big dumps caution → Mild ups → Spikes FADE → Stuck

RotoWire DNP: only mark when lineup is solid AND name clearly missing
...
.how-to{background:linear-gradient(135deg,#1a0f28,#2a1040);border:1px solid #f472b6;border-radius:14px;padding:12px 14px;margin-bottom:12px;font-size:.82rem;line-height:1.45;position:relative}
...
PENDING_PAGE = 20
MGM_STAY_SNAPS = 3
LINEUP_MIN_NAMES = 80  # if scrape returns fewer, treat as incomplete — do NOT auto-DNP
...
def names_match(a, b):
    """Strict-ish match for DNP safety — prefer True only when clearly same player."""
    ...
    if len(pa) >= 2 and len(pb) >= 2:
        ...
        if pa[-1] == pb[-1] and pa[0][0] == pb[0][0] and (len(pa[0]) == 1 or len(pb[0]) == 1 or pa[0][:3] == pb[0][:3]):
            return True
        # one first contains the other (Jose vs José stripped already)
...
def name_in_lineup(player, lineup_names):
    """
    True  = clearly in lineup
    False = clearly NOT in lineup (only when we have a solid list)
    None  = unknown / incomplete list — NEVER auto-DNP on None
    """
    if not lineup_names:
        return None
    if len(lineup_names) < LINEUP_MIN_NAMES:
        return None  # scrape incomplete — do not trust "missing"
    ...
    for ln in lineup_names:
        if names_match(cn, ln):
            return True
    # last-name + first-initial backup
    parts = cn.split()
...
def build_movement_board(lock=None, live_df=None):
    ...
    for player, entry in (lock or {}).items():
        ...
        card = {
            ...
            "is_big_dump": delta <= -BIG_MOVE,
            "is_good_down": -BIG_MOVE < delta <= -40,
            "is_mild_up": 40 <= delta < BIG_MOVE,
            "is_stuck": abs(delta) < 25,
        }
        if card["is_spike"]:
            spikes.append(card)
...
def auto_mark_dnp():
    """Only DNP when lineup is solid AND player clearly missing. Never on incomplete scrape."""
    ...
    for r in rows:
        if r.get("date") != today or r.get("result") != "PENDING" or r.get("source") != "take_it":
            continue
        status = name_in_lineup(r.get("player", ""), lineup)
        if status is False:  # only False — never None
            r["result"] = "DNP"
            n += 1
    if n:
        save_results(rows)
...
def bulk_miss_to_dnp(today_only=True):
    lineup = st.session_state.get("lineup_names") or set()
    if not lineup or len(lineup) < LINEUP_MIN_NAMES:
        return 0, "Lineup incomplete or missing — not converting (need solid RotoWire pull)"
    rows = load_results()
    today = today_az()
    n = 0
    for r in rows:
...
def log_bet_this(ev_board):
    ...
    for item in ev_board:
        if not item.get("is_bet"):
            continue
        # Only skip if clearly NOT in lineup (False). Unknown (None) still logs.
...
def fetch_rotowire_lineups():
    """
    More robust scrape. Returns names only if we get a solid count.
    Incomplete scrapes must NOT drive auto-DNP.
    """
    if not HAS_BS4:
    ...
    try:
        ...
        selectors = [
            ...
            "div.lineup__player span.player-name",
            ".lineup__player a",
        ]
...
def run_flags(df, previous_df=None, record_history=True, selected_events=None):
    ...
    for (player, _), g in df.groupby(["player", "point"], dropna=False):
        # Only hide if clearly NOT in lineup
...
def main():
    ...
    st.markdown(
        ...
        f'<b>📈 Move</b> ordered likes→spikes · 🔒 {lock_n}'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    <div class="petty-row">
      ...
      <div class="petty-box"><div class="petty-num">{n_moves}</div><div class="petty-label">📈 MOVES</div></div>
    ...
    with tab_move:
        ...
        st.caption(
            f"**Order:** Likes → Good downs → Big dumps → Mild ups → Spikes (FADE) → Stuck · "
            f"Only **+{PRICE_MIN_MOVE}+** · ±{BIG_MOVE} spike = hate · moderate down = like"
        )
        lock = st.session_state.get("pregame_lock") or load_pregame()
        if not lock:
            ...
            _section("🚫 Spikes (+150+) — FADE / we hate this", m["spikes"])
    ...
    with tab_tricks:
        ...
        sub = st.tabs(["🎯 DK", "📉 Was DK", "🎰 MGM", "⭐ Exact MGM", "🤝 Exact", "💙 FD", "💚 365", "🔒 Lock"])
        with sub[0]:
    ...
    with tab_results:
        ...
        with st.expander(f"DNP ({len(dnps)}) — fix false ones with ↩️ Undo on graded or re-mark", expanded=False):
            st.caption("If someone here is actually playing, open All graded / Pending and fix manually. Auto-DNP only runs when RotoWire returns ≥80 names.")
            for r in sorted(dnps, key=lambda x: x.get("player") or ""):
                rid = r.get("id", "")
                st.markdown(f"🟡 **{r.get('player')}** · was {format_odds(r.get('best_price'))}")
                if rid and st.button("↩️ back to PENDING", key=f"dnp_undo_{rid}"):
    ...
    with tab_gloss:
        ...
        <div class="how-to">
            ...
            <b>📈 Movement</b> = 500+ · Likes → downs → dumps → ups → spikes → stuck.<br>
            <b>No Caesars.</b> Edge is info only. RotoWire DNP only when lineup is solid (≥80 names).
        </div>
        """, unsafe_allow_html=True)
...
Board order: HOT → TAKE → PASS (mobile row-by-row).
            """)

        with st.expander("🎰 MGM · 💚 Bet365"):
            st.markdown("""
Digits **only** on MGM + Bet365. PAIR first. TRIO only if no pair.  
MGM Exact = same full price, same team (PAIR).  
Stayed / Last one left = boost only.
            """)

        with st.expander("🎯 DK · 💙 FD · HardRock"):
            st.markdown(f"""
**DK 10** · **Was DK 10** · **DK FD-style** (≥ +{FD_MIN} FD endings)  
**FD Pattern** / **FD 600**  
**HardRock best** → PASS
            """)
...
3. ⚠️ Big dumps (−150+) — value can die  
4. ⬆️ Mild ups (+40 to +149) — watch  
5. 🚫 Spikes (+150+) — **FADE / hate**  
6. 😴 Stuck same all day — noise  

Only **+{PRICE_MIN_MOVE}+** · needs 2+ fetches.
            """)

        with st.expander("📋 Cheat Sheet · 📊 Results · RotoWire"):
            st.markdown(f"""
Cheat Sheet: paste My / Girls lists → method hits.  
HIT% ignores DNP. Best-book HIT% = MGM · DK · FD · HardRock only.  

**RotoWire / DNP safety**  
- Auto-DNP **only** if scrape returns **≥ {LINEUP_MIN_NAMES}** names  
- Incomplete scrape → auto-DNP **OFF** (you’ll see a warning)  
- False DNP → use **↩️ back to PENDING** under the DNP list
            """)

        with st.expander("What we ignore"):
            st.markdown("- Caesars · non-MGM/365 digits · edge as a gate · mirrors · sub-500 movement noise")

        with st.expander("How to run a slate"):
            st.markdown(f"""
1. Load Games → select → Fetch (builds lock)  
2. Fetch again later for Movement  
3. RotoWire **after** lineups post — check caption says auto-DNP ON  
4. Board HOT → TAKE → PASS  
5. Move tab for likes / fades  
...
def main():
    ...
    with tab_gloss:
        ...
        <div class="how-to">
            ...
            <b>🟢 TAKE IT</b> = live pair/trio · not faded · price high enough · not HardRock-best.<br>
            <b>⚪ PASS</b> = no pair/trio · HardRock is best · big move (±150) · price too low.<br>
            <b>📈 Movement</b> = 500+ only · Up red · Down green · FD 10–100 under MGM = like.<br>
            <b>No Caesars.</b> Edge is info only — not a TAKE/PASS gate.
        </div>
        """, unsafe_allow_html=True)
