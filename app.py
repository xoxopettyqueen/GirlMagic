...
TAKE IT requires ALL THREE families:
  ≥1 DK  +  ≥1 MGM  +  ≥1 FD
  plus edge ≥ EDGE_MIN

Also: pregame lock · Results undo · all pending paged · Log HR · glossary
"""
...
# ── family classifiers (TAKE IT needs all 3) ─────────────────
def is_dk_method(m):
    m = str(m)
    if m == "DK 10" or m.startswith("DK "): return True
    if "DK" in m and "style" in m.lower(): return True
    return False
...
def is_mgm_method(m):
    ...
    if m.startswith("Match "): return True  # digit pairs are MGM-side
    return False
...
def family_flags(meths):
    has_dk = any(is_dk_method(m) for m in meths)
    has_mgm = any(is_mgm_method(m) for m in meths)
    has_fd = any(is_fd_method(m) for m in meths)
    return has_dk, has_mgm, has_fd

def has_dk_mgm_fd(meths):
    d, m, f = family_flags(meths)
    return d and m and f

def family_gap_text(meths):
    d, m, f = family_flags(meths)
    missing = []
    if not d: missing.append("DK")
    if not m: missing.append("MGM")
    if not f: missing.append("FD")
    if not missing:
        return "DK+MGM+FD ✓"
    return "need " + "+".join(missing)

def has_dk_or_mgm(meths):
    return any(is_dk_method(m) or is_mgm_method(m) for m in meths)

def method_tag_class(m):
    m = str(m)
    if is_dk_method(m): return "tag-dk"
    if is_mgm_method(m): return "tag-mgm"
    if is_fd_method(m): return "tag-fd"
    if m.startswith("B365"): return "tag-b365"
    if m in ("Exact Match",) or m.startswith("Match "): return "tag-match"
...
def girl_magic_score(core_count, edge, methods):
    ...
    if has_dk_mgm_fd(methods): bonus += 5
...
def run_flags(df, previous_df=None, record_history=True, selected_events=None):
    ...
    # FD Pattern — needs DK or MGM already on player
    for _, row in df.iterrows():
    ...
    # Optional DK FD-style: DK price that mirrors FD pattern endings
    for _, row in df.iterrows():
        ...
        if price >= FD_MIN and last in (10, 20, 30, 60, 70, 90):
            results.append({"type": "dk", "label": row["player"],
                ...
                "event": row["event"], "css": "dk", "methods": ["DK FD-style"]})
    ...
    # ── THE BOARD: TAKE IT = edge + DK + MGM + FD ────────────
    ev_board = []
    ...
    for (player, _), g in df.groupby(["player", "point"], dropna=False):
        ...
        families_ok = has_dk_mgm_fd(display_meths)
        # ★ TAKE IT only if DK + MGM + FD all present AND edge ok
        is_bet = edge >= EDGE_MIN and families_ok
        score = girl_magic_score(core_count, edge, display_meths)
        conf, bars, level = get_confidence(score, is_bet)
        gap = family_gap_text(display_meths)
        if is_bet:
            why = f"Score {score}/100 · DK+MGM+FD · Edge {int(edge)}. TAKE IT."
        else:
            why = f"Score {score}/100 · Edge {int(edge)} · {gap}."
        ev_board.append({
    ...
    for player, old in prev_ev.items():
        ...
        if player not in all_players_now:
            reasons.append("Line gone from live feed")
            if lock_note:
...
def main():
    ...
    st.markdown(f"""
    <div class="how-to">
        🟢 <b>TAKE IT</b> = <b>DK + MGM + FD</b> (all three families) <b>and</b> edge ≥ {EDGE_MIN}<br>
        🔒 Pregame lock: <b>{lock_n}</b> · snaps: <b>{hist_n}</b> · lineup: <b>{lu_n}</b> · ↩️ Undo on Results
    </div>
    """, unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        st.caption("🟢 TAKE IT only when **DK + MGM + FD** all fire and edge ≥ {}. PASS shows gaps.".format(EDGE_MIN))
        if not lu_n:
            st.caption("⚠️ Refresh RotoWire.")
        if not ev_board:
            ...
            for idx, item in enumerate(ev_board):
                with cols[idx % 2]:
                    ...
                    <div class="card {cls} grid-card">
                        ...
                        Consensus: {format_odds(item["median"])} · Edge: {int(item["edge"])}<br>
                        <b>Families:</b> {fam}<br>
    ...
    show(tabs[1], "dk", "🎯 DraftKings", "DK 10 + DK FD-style.")
    show(tabs[2], "mgm", "🎰 BetMGM", "Same-team pairs/groups.")
    show(tabs[3], "match", "🤝 Exact Match", "Same price across books.")
    show(tabs[4], "mgm_exact", "⭐ MGM Exact", "Same MGM price, same team.")
    show(tabs[5], "digit", "🔢 Digits", "Pairs/groups · same team · 25/50/75.")
    show(tabs[6], "fd", "💙 FanDuel", f"≥ +{FD_MIN} or +600 with DK/MGM.")
    show(tabs[7], "b365", "💚 Bet365", "850s · pairs 25/50/75.")
    ...
    with tabs[13]:
        ...
        if not lock:
            ...
            for player, entry in items:
                ...
                with cols[shown % 2]:
                    st.markdown(f'<div class="card grid-card"><b>{player}</b><br>' + "<br>".join(lines) + "</div>", unsafe_allow_html=True)
                shown += 1
    ...
    with tabs[19]:
        ...
        st.caption("Wrong HIT → **Recent graded → ↩️ Undo** (back to PENDING).")
        ...
        with lc2:
            hr_price = st.text_input("Price (blank=lock)", key="hr_price")
        with lc3:
            hr_book = st.selectbox("Book", ["betmgm", "bet365", "draftkings", "fanduel", "hardrockbet", "caesars", "untagged"], key="hr_book")
        with lc4:
            st.write(""); st.write("")
            ...
            for r in slice_p:
                ...
                with c1:
                    if st.button("🟢 HIT", key=f"hit_{rid}"):
                        set_result_status(rid, "HIT"); st.rerun()
                with c2:
                    if st.button("🔴 MISS", key=f"miss_{rid}"):
                        set_result_status(rid, "MISS"); st.rerun()
        ...
        if not done:
            ...
            for r in reversed(done[-40:]):
                ...
                st.markdown(f"{icon} **{r['player']}** · {format_odds(r.get('best_price'))} {book_label(r.get('best_book'))}{end_s}{src}")
    ...
    with tabs[20]:
        ...
        with st.expander("🟢 TAKE IT rule (updated)", expanded=True):
            st.markdown(f"""
**TAKE IT** only when **all** of these are true:

1. **≥1 DK tag** — `DK 10` or `DK FD-style`  
2. **≥1 MGM tag** — `MGM 00/25/50/75`, `MGM Exact`, `Match 25/50/75`, `Stayed in the group`, `Last one left`  
3. **≥1 FD tag** — `FD Pattern` or `FD 600`  
4. **Edge pts ≥ {EDGE_MIN}**  
5. **2+ core methods** overall · Over **0.5 HR** · preferably in RotoWire lineup  

PASS still shows on The Board with **Families: need DK / need MGM / need FD** so you see what’s missing.
            """)
        with st.expander("🔒 Pregame lock"):
            st.markdown("Every fetch merges prices. When MGM vanishes after live, lock **keeps** last numbers. Use for Log a HR.")
        with st.expander("📊 Results · Undo"):
            st.markdown("Grade **all** PENDING (oldest first, page through). **↩️ Undo** on graded → PENDING (or deletes manual HR).")
        with st.expander("Core methods"):
            st.markdown("DK 10s · MGM groups · Exact · Digits · FD patterns · B365 when available · Multi-book Shorten counts; Lengthen is noise.")

    st.markdown('<div class="footer">👑 Girl Magic • DK + MGM + FD to TAKE IT</div>', unsafe_allow_html=True)
